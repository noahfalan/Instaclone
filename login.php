<!DOCTYPE html>
<html>
  <head>
    <meta charset="utf-8">
    <link rel="stylesheet" href="css/login.css" type="text/css">
    <title>Instagrammas</title>
    <link href="https://fonts.googleapis.com/css?family=Lato" rel="stylesheet">
    <link href="https://fonts.googleapis.com/css?family=Abril+Fatface" rel="stylesheet">
    <link rel="shortcut icon" href="css/Images/instalogo.png">
  </head>
  <body>
    <nav>
      <a href="index.php"><h1>Instaclone</h1></a>
      <a href="login.php"><img src="css/Images/log-in.png" alt="profile" height="32px" width="32px"></a>
      <a href="upload.php"><img src="css/Images/upload.png" alt="profile" height="32px" width="32px"></a>
      <a href="login.php"><img src="css/Images/user.png" alt="profile" height="32px" width="32px"></a>
    </nav>

<div class="menu">
  <h2>Login</h2>
  <img src="data:image/png;base64," class="registerpicture">
  <p>Don't have an account? Please create one <a href="register.php">here!</a></p>
  <input class="box" type="text" name="loginname" placeholder="Name">
  <br><br>
  <input class="box" type="text" name="emaillogin" placeholder="Email">
</div>


</body>
</html>
